...
describe('Check amazon customer in back office', function() {
    ...
    describe('Access to the customer page', function (done) {
        ...
        it('should filter the list by address email', function (done) {
            this.client
                ...
                .setValue(this.selector.BO.CustomersPage.customer_address_email_input, 'prestotests+amazon@gmail.com')
                .call(done);
        });
        ...
        it('should check new amazon customer', function (done) {
            this.client
                .waitForExist(this.selector.BO.Amazon.logo_customer_page, 90000)
                .moveToObject(this.selector.BO.Amazon.logo_customer_page)
                .getAttribute(this.selector.BO.Amazon.logo_customer_page, 'title').then(function (title) {
                should(title).be.equal('Amazon');
            })
                .getAttribute(this.selector.BO.Amazon.logo_customer_page, 'alt').then(function (alt) {
                should(alt).be.equal('Amazon');
            })
                .call(done);
        });
