'use strict';
var should = require('should');
var common = require('../../../../common/common.webdriverio');
var globals = require('../../../../common/globals.webdriverio');
var externals = require('../../../../common/external_globals.webdriverio');

describe('Configuration app of vkontakte in back office', function() {
    common.initMocha.call(this);

    before(function (done) {
        this.selector = globals.selector;
        this.external = externals.selector;
        this.client.call(done);
    });

    after(common.after);

    describe('Log in in Back Office', function (done) {
        it('should log in successfully in BO', function (done) {
            global.fctname = this.test.title;
            this.client
                .url('https://' + URL + '/backoffice/')
                .waitForExist(this.selector.BO.AccessPage.login_input, 120000)
                .setValue(this.selector.BO.AccessPage.login_input, email)
                .waitForExist(this.selector.BO.AccessPage.password_input, 120000)
                .setValue(this.selector.BO.AccessPage.password_input, password)
                .waitForExist(this.selector.BO.AccessPage.login_button, 90000)
                .click(this.selector.BO.AccessPage.login_button)
                .waitForExist(this.selector.BO.Common.menu, 60000)
                .call(done);
        });
    });

    describe('Access to the module configuration page', function (done) {
        it('should go to modules installed page', function (done) {
            this.client
                .click(this.selector.BO.ModulesPage.modules_subtab)
                .waitForExist(this.selector.BO.ModulesPage.page_loaded, 90000)
                .call(done);
        });

        it('should go to the module', function (done) {
            if (global.nbr == 0) {
                done(new Error('The module you are searching for does not exist!'));
            }
            else {
                this.client
                    .setValue(this.selector.BO.ModulesPage.search_input, module_tech_name_socialConnect)
                    .click(this.selector.BO.ModulesPage.search_button)
                    .call(done);
            }
        });

        it('should click on configuration button', function (done) {
            if (global.nbr == 0) {
                done(new Error('The module you are searching for does not exist!'));
            }
            else {
                this.client
                    .waitForExist(this.selector.BO.ModulesPage.module_tech_name, 90000)
                    .waitForExist(this.selector.BO.ModulesPage.configuration_button,90000)
                    .click(this.selector.BO.ModulesPage.configuration_button)
                    .call(done);
            }
        });
    });

    describe('Access to the configuration page', function (done) {
        it('should access to vkontakte configuration page', function (done) {
            this.client
                .waitForExist(this.selector.BO.Vkontakte.config_button, 90000)
                .click(this.selector.BO.Vkontakte.config_button)
                .call(done);
        });

        it('should go to vkontakte developers link', function (done) {

            this.client
                .waitForExist(this.selector.BO.Vkontakte.site_address_input, 90000)
                .getAttribute(this.selector.BO.Vkontakte.site_address_input, 'value').then(function (url) {
                global.site_address = url;
                global.base_domain = url.split('https://').pop().split('/').shift();
            })
                .waitForExist(this.selector.BO.Vkontakte.developers_link, 90000)
                .click(this.selector.BO.Vkontakte.developers_link)
                .pause(3000)
                .call(done);
        });

        it('should access to the vkontakte account', function (done) {
            global.fctname = this.test.title;
            this.client
                .getTabIds().then(function (handles) {
                return this.switchTab(handles[handles.length - 1])
            })
                .waitForExist(this.external.FO.Vkontakte.username_input, 90000)
                .setValue(this.external.FO.Vkontakte.username_input, 'prestotestsvkontakte@gmail.com')
                .setValue(this.external.FO.Vkontakte.password_input, 'presto_tests')
                .waitForExist(this.external.FO.Vkontakte.signin_button, 90000)
                .click(this.external.FO.Vkontakte.signin_button)
                .pause(5000)
                .call(done);
        });

        it('should access to the list of apps', function (done) {
            this.client
                .waitForExist(this.external.FO.Vkontakte.my_apps_menu, 90000)
                .click(this.external.FO.Vkontakte.my_apps_menu)
                .call(done);
        });

        it('should access to the app PrestoAppSocialConnect', function (done) {
            this.client
                .waitForExist(this.external.FO.Vkontakte.manage_button, 90000)
                .click(this.external.FO.Vkontakte.manage_button)
                .call(done);
        });

        it('should click on settings menu', function (done) {
            this.client
                .waitForExist(this.external.FO.Vkontakte.settings_subtab, 90000)
                .click(this.external.FO.Vkontakte.settings_subtab)
                .pause(3000)
                .waitForExist(this.external.FO.Vkontakte.id_text, 90000)
                .getText(this.external.FO.Vkontakte.id_text).then(function (key) {
                global.vkontakte_customer_key = key;
            })
                .getAttribute(this.external.FO.Vkontakte.secret_key_input, 'value').then(function (secret) {
                global.vkontakte_customer_secret = secret;
            })
                .call(done);
        });

        it('should enter the site address', function (done) {
            this.client
                .waitForExist(this.external.FO.Vkontakte.site_url_input, 90000)
                .setValue(this.external.FO.Vkontakte.site_url_input, global.site_address)
                .call(done);
        });

        it('should enter the base domain', function (done) {
            this.client
                .waitForExist(this.external.FO.Vkontakte.delete_base_domain_icon, 90000)
                .click(this.external.FO.Vkontakte.delete_base_domain_icon)
                .pause(3000)
                .waitForExist(this.external.FO.Vkontakte.domain_input, 90000)
                .setValue(this.external.FO.Vkontakte.domain_input, global.base_domain)
                .call(done);
        });

        it('should click on save button', function (done) {
            this.client
                .moveToObject(this.external.FO.Vkontakte.save_button)
                .click(this.external.FO.Vkontakte.save_button)
                .pause(5000)
                .getTabIds().then(function(handles){
                return this.switchTab(handles[0])
            })
                .call(done);
        });

        it('should enter the customer id and secret', function (done) {
            this.client
                .waitForExist(this.selector.BO.Vkontakte.customer_key_input, 90000)
                .setValue(this.selector.BO.Vkontakte.customer_key_input, global.vkontakte_customer_key)
                .setValue(this.selector.BO.Vkontakte.customer_secret_input, global.vkontakte_customer_secret)
                .call(done);
        });

        it('should save vkontakte configurations', function (done) {
            this.client
                .waitForExist(this.selector.BO.Vkontakte.save_button, 90000)
                .click(this.selector.BO.Vkontakte.save_button)
                .call(done);
        });
        it('logout BO', function (done) {
            this.client
                .signoutBO()
                .call(done);
        });
    });
});
