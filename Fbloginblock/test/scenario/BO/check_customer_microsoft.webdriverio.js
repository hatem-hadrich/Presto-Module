...
describe('Check microsoft customer in back office', function() {
    ...
    describe('Access to the customer page', function (done) {
        ...
        it('should filter the list by address email', function (done) {
            this.client
                ...
                .setValue(this.selector.BO.CustomersPage.customer_address_email_input, 'prestotests@outlook.com')
                .call(done);
        });
        ...
        it('should check new microsoft customer', function (done) {
            this.client
                .waitForExist(this.selector.BO.Microsoft.logo_customer_page, 90000)
                .moveToObject(this.selector.BO.Microsoft.logo_customer_page)
                .getAttribute(this.selector.BO.Microsoft.logo_customer_page, 'title').then(function (title) {
                should(title).be.equal('Microsoft');
            })
                .getAttribute(this.selector.BO.Microsoft.logo_customer_page, 'alt').then(function (alt) {
                should(alt).be.equal('Microsoft');
            })
                .call(done);
        });
