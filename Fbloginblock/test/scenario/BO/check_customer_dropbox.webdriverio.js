...
describe('Check dropbox customer in back office', function() {
    ...
    describe('Access to the customer page', function (done) {
        ...
        it('should filter the list by address email', function (done) {
            this.client
                ...
                .setValue(this.selector.BO.CustomersPage.customer_address_email_input, 'prestotests+dropbox@gmail.com')
                .call(done);
        });
        ...
        it('should check new dropbox customer', function (done) {
            this.client
                .waitForExist(this.selector.BO.Dropbox.logo_customer_page, 90000)
                .moveToObject(this.selector.BO.Dropbox.logo_customer_page)
                .getAttribute(this.selector.BO.Dropbox.logo_customer_page, 'title').then(function (title) {
                should(title).be.equal('Dropbox');
            })
                .getAttribute(this.selector.BO.Dropbox.logo_customer_page, 'alt').then(function (alt) {
                should(alt).be.equal('Dropbox');
            })
                .call(done);
        });
