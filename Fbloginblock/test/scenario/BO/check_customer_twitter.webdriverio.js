...
describe('Check twitter customer in back office', function() {
    ...
    describe('Access to the customer page', function (done) {
        ...
        if(global.number_of_customers >= 1) {
            it('should filter the list by address email', function (done) {
                this.client
                    ...
                    .setValue(this.selector.BO.CustomersPage.customer_address_email_input, 'prestotests+twitter@gmail.com')
                    .call(done);
            });
        ...
        it('should check new twitter customer', function (done) {
            this.client
                .waitForExist(this.selector.BO.Twitter.logo_customer_page, 90000)
                .moveToObject(this.selector.BO.Twitter.logo_customer_page)
                .getAttribute(this.selector.BO.Twitter.logo_customer_page, 'title').then(function (title) {
                should(title).be.equal('Twitter');
            })
                .getAttribute(this.selector.BO.Twitter.logo_customer_page, 'alt').then(function (alt) {
                should(alt).be.equal('Twitter');
            })
                .call(done);
        });
