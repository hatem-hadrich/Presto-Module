...
describe('Check yahoo customer in back office', function() {
    ...
    describe('Access to the customer page', function (done) {
        ...
        it('should filter the list by address email', function (done) {
            this.client
                ...
                .setValue(this.selector.BO.CustomersPage.customer_address_email_input, 'prestotests@yahoo.com')
                .call(done);
        });
        ...
        it('should check new yahoo customer', function (done) {
            this.client
                .waitForExist(this.selector.BO.Yahoo.logo_customer_page, 90000)
                .moveToObject(this.selector.BO.Yahoo.logo_customer_page)
                .getAttribute(this.selector.BO.Yahoo.logo_customer_page, 'title').then(function (title) {
                should(title).be.equal('Yahoo');
            })
                .getAttribute(this.selector.BO.Yahoo.logo_customer_page, 'alt').then(function (alt) {
                should(alt).be.equal('Yahoo');
            })
                .call(done);
        });
