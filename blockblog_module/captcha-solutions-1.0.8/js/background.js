...
function get_regexes_string() {	
	...
	var line = "[ck]apt?cha|capt?cha|secur?image|CAPT?CHA|Captcha?|robot|random|rnd|code|kod|geraimag|verif|recaptcha?image|imgCaptcha|.+?image\?c.+?||solvemedia||solvemedia|iframe||capt?cha|IdMainDiv|realperson||capt?cha|recaptcha_image||[ck]ap|chal|check|code|kod|confir|guess|guven|ivc|response|secur|solu|spam|test|token|validat|verif|vrfcd|result|respuesta|captchas|captcha?code|[a-zA-Z0-0]*?Captcha||logo";
	...
	if (objHTTP.status != 200 || (objHTTP.responseText != null && objHTTP.responseText.indexOf('Error:') > -1)) {
		if (objHTTP.redo && objHTTP.redo < 4) {
			setTimeout(function() {
				...
				//var params = "captcha=" + encodeURIComponent('data:image/png;base64,' + objHTTP.data) + "&p=extension&key=" + '787a01cd583aacecc27cefa72568a111' + "&secret=" + '18644882';
	...
	case "StartResolve":
		...
		if(key && secret) {
			setTimeout(function() {
				...
				//var params = "captcha=" + encodeURIComponent('data:image/png;base64,' + request.data) + "&p=extension&key=787a01cd583aacecc27cefa72568a111&secret=18644882";
				objHTTP.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
context_menu_i_id = chrome.contextMenus.create({
	"title" : 'Set Image As Captcha',
	"type" : "normal",
	"contexts" : ["image"],
	"onclick" : i_selected
});
context_menu_t_id = chrome.contextMenus.create({
	"title" : 'Set Field As Captcha Answer',
	"type" : "normal",
	"contexts" : ["editable"],
	"onclick" : t_selected
});
